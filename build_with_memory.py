    ...
    if pid == 0:
        ...
        try:
            while True:
                ...
                elif password_sent and not build_sent and ("master" in log or "$" in log):
                    time.sleep(2)
                    print("\n[AI] Attempting build with increased memory...")
                    
                    # Run build with increased memory limit
                    os.write(fd, b"cd applications/zwpneuuzgz/public_html && export NODE_OPTIONS=--max-old-space-size=4096 && npm run build\n")
